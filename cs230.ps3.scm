...
(define helper2
  (lambda (graph lst last)
     (cond ((null? lst) '())
           ((member? last (name-vertices(exits (lookup-vertex (helper1 lst) (vertices graph)) graph))) (append lst (list last)))
           (else (append lst (name-vertices(exits (lookup-vertex (helper1 lst) (vertices graph)) graph)))))))

 (define path?
   (lambda (v1 v2 graph)
     (letrec ((loop
               (lambda(vxs count) ;one of these tracks vertices we have been to and the other is a counter
                 (cond ((null? (vertices graph)) #f)
                       ((null? vxs) #f)
                       ((equal-vertex? (lookup-vertex v1 (vertices graph))(lookup-vertex v2 (vertices graph))) #t)
                       ((member? v2 vxs) #t)
                       ((zero? count) #f)
                       (else (loop (helper2 graph vxs v2) (- count 1)))))))
       (loop (list v1) (length (vertices graph))))))
                       
(define helper1
  (lambda (graph states visited final)
    (cond ((member-vertices final states) states)
          (else
           (let* ((neighbors (union visited states))
               (nextnodes (map finish (filter (lambda (e) 
                                          (member-vertices (start e) states))
                                        (edges graph))))
              (left (set-diff-vertices nextnodes neighbors)))
            (if(null? left) states
               (helper1 graph (union neighbors left) neighbors final)))))))


(define path?
  (lambda (v1 v2 graph)
    (let ((ends (helper1 graph (list(lookup-vertex v1 (vertices graph))) `() (lookup-vertex v2 (vertices graph)))))
      (if(eq? #f (member-vertices (lookup-vertex v2 (vertices graph)) ends)) #f #t)
            )))
